...
class LXDAPISession(object):
    ...
    def profile_show(self, instance):
        """Validate if the profile is available on the LXD
           host

           :param instance: nova instance object
        """
        LOG.debug('profile_show called for instance',
                  instance=instance)
        try:
            ...
            client = self.get_session()
            return client.profile_show(instance.name)[1]['metadata']
        except lxd_exceptions.APIError as ex:
            msg = _('Failed to communicate with LXD API %(instance)s:'
                    ' %(reason)s') % {'instance': instance.name,
                                      'reason': ex}
            ...
            with excutils.save_and_reraise_exception():
                LOG.error(
                    _LE('Failed to show profile %(instance)s: '
                        '%(reason)s'),
                    {'instance': instance.name, 'reason': ex})
