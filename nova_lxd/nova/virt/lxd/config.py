...
class LXDContainerConfig(object):
    ...
    def create_profile(self, instance, network_info, flavor=None, ptp_device_path=None):
        """Create a LXD container profile configuration

        :param ptp_device_path: LXD ptp device path
        :param flavor: nova flavor object
        :param instance: nova instance object
        ...
        try:
            ...
            config['config'] = self.create_config(instance_name, instance, flavor)
            ...
            if ptp_device_path is not None:
                disk_device = {
                    'sharedtmp': {
                        'path': '%s' % ptp_device_path,
                        'source': '%s' % ptp_device_path,
                        'type': 'disk'
                    }
                }
                config['devices'].update(disk_device)
    ...
    def create_config(self, instance_name, instance, flavor):
        ...
        try:
            ...
            # Set the instanisce memory limit
            if flavor is None:
    ...
    def config_instance_options(self, config, instance):
        ...
        config['environment.product_name'] = str(instance.uuid)
    ...
    def configure_container_root(self, instance):
        ...
        try:
            ...
            elif 'ceph' in str(lxd_config['storage']):
                config['root'] = {'path': '/', 'pool': 'pool',
                                  'type': 'disk'}
            else:
                config['root'] = {'path': '/', 'pool': 'default',
                                  'type': 'disk'}
            return config
        except Exception as ex:
            with excutils.save_and_reraise_exception():
    ...
    def create_container_net_device(self, instance, vif):
        ...
        try:
            ...
            config = {}
            key = str(network_config['bridge'])
            config[key] = {
                'nictype': 'bridged',
                'hwaddr': str(network_config['mac_address']),
                'parent': key,
                'type': 'nic'}

            host_device = self.vif_driver.get_vif_devname(vif)
            if host_device:
                config[key]['host_name'] = host_device
            return config
        except Exception as ex:
